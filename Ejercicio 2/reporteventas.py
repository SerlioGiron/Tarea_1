...
def obtener_municipio(rand_departamento):
    municipios_por_departamento = {
        1: (1, 8), 2: (1, 10), 3: (1, 21), 4: (1, 23), 5: (1, 12), 
        6: (1, 16), 7: (1, 19), 8: (1, 28), 9: (1, 9), 10: (1, 17), 
        11: (1, 4), 12: (1, 19), 13: (1, 28), 14: (1, 16), 
        15: (1, 23), 16: (1, 28), 17: (1, 9), 18: (1, 11)
    }
    return generar_random(*municipios_por_departamento[rand_departamento])
...
def obtener_dia_compra(rand_mes_fecha):
    ...
    return generar_random(1, dias_por_mes[rand_mes_fecha])
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Debe de correr el programa de la siguiente forma: python ejercicio2_python.py numVentas")
        sys.exit(1)
    num_ventas_generar = int(sys.argv[1])
    random.seed(time.time())
    nombre_sucursales = ["La Colonia Las Hadas", "La Colonia La Kennedy", "La Colonia La Granja", "La Colonia La Hacienda", "La Colonia Galerias", "La Colonia Villas del Sol", "La Colonia Anillo Periferico", "La Colonia Las Torres"]
    ventas = []
    ...
    for _ in range(num_ventas_generar):
        ...
        sucursal = nombre_sucursales[generar_random(0, 7)]
